<template>
    <div class="panel">
        <div class="flex items-center justify-between mb-5">
            <h5 class="font-semibold text-lg dark:text-white-light">Project: {{ }}</h5>
            <button class="btn btn-primary flex" type="button" @click="addEditTask()">
        ...
        <div class="mb-5">
            <ul class="font-semibold">
                <li class="py-[5px]" v-for="taskk in taskks" :key="taskk.id" :value="taskk.id">
                    <div class="flex items-center">
                        ...
                        <span class="ml-60">{{ taskk.status.status_name }}</span>
                        <button class="ml-60">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-primary inline relative -top-1 ltr:mr-2 rtl:ml-2">
                                <path opacity="0.5"
...
const fetchTaskks = async () => {
  try {
    const response2 = await axios.post('/api/task/all'); // Replace with your API endpoint
    taskks.value = response2.data;
    console.log(taskks.value);
  } catch (error) {
    console.error('Error fetching assignees:', error);
    // Handle API call errors gracefully
  }
